...
FastAPI service orchestrates the pipeline: Azure Document Intelligence (parse) → structure-aware chunker → OpenAI dense embeddings + local Splade sparse embeddings → Qdrant index with named vectors and version payload → Qdrant prefetch + RRF fusion → Cohere rerank → grounded LLM generation → response with `citations[]` carrying full lineage (page, section_path, bbox, chunk_text, score). All provider calls (OpenAI, Cohere) flow through Bifrost AI Gateway. LLM Guard sidecar scans both input and output (groundedness via NLI). Redis caches query embeddings, rerank results, and final answers. Logfire + structlog provide observability. RAGAS eval endpoint validates groundedness on a bundled golden set.
...
- **Layout:** `src/rag_hackathon/` package with submodules `api/`, `ingestion/`, `retrieval/`, `generation/`, `security/`, `gateway/`, `cache/`, `versioning/`, `eval/`, `observability/`, `core/`. `tests/` mirrors `src/` structure.
- **Strategy boundaries:** every swappable component (chunker, embedder, retriever, reranker, generator, guard, gateway client) defines its `Protocol` in `protocols.py` inside its submodule. Default impl lives next to it. Routes never import concrete implementations directly — they pull off `app.state`.
...
- **All provider traffic goes through `gateway.bifrost.BifrostClient`.** Chat + embeddings use `AsyncOpenAI(base_url=f"{bifrost_url}/openai")`. Cohere rerank uses an `httpx` POST to `{bifrost_url}/v1/rerank` (root path, **not** under `/openai`).
...
- **Splade is asymmetric.** Use `SPLADE_DOC_MODEL` for ingestion-side embeddings, `SPLADE_QUERY_MODEL` for query-side. Mixing them silently degrades retrieval quality.
...
Listed in full in U1 of the plan. Required (no defaults, app fails to start without them): `AZURE_DI_ENDPOINT`, `AZURE_DI_KEY`, `OPENAI_API_KEY`, `COHERE_API_KEY`, `LOGFIRE_TOKEN`. Sidecar URLs and tunables have sensible defaults — see `Settings` in `src/rag_hackathon/core/settings.py` (added in U1) and `.env.example`.
