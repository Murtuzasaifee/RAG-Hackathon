...
- **All provider traffic goes through `gateway.bifrost.BifrostClient`.** Bifrost routes by model prefix: `openai/text-embedding-3-small` → OpenAI directly, `meshapi/gpt-5.4` → MeshAPI (`https://api.meshapi.ai`), `cohere/rerank-*` → Cohere. The `BifrostClient` auto-prefixes: `embed()` adds `openai/`, `chat()` adds `{LLM_PROVIDER}/` (default `meshapi`), `rerank()` uses the cohere provider directly. Bifrost config (`docker/bifrost/config.json`) defines three providers with `custom_provider_config.allowed_requests` restricting meshapi to chat-only. Cohere rerank uses an `httpx` POST to `{bifrost_url}/v1/rerank` (root path, **not** under `/openai`).
